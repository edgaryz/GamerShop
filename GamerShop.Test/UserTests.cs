...
namespace GamerShop.Test
{
    public class UserTests
    {
        ...
        public async Task GetUserListTest()
        {
            ...
            _usersRepository.Setup(x => x.GetTotalUserCount()).Returns(Task.FromResult(userList.Count));
            _usersMongoRepository.Setup(x => x.GetUserCount()).Returns(Task.FromResult(0L));
            _usersRepository.Setup(x => x.GetAllUsers()).Returns(Task.FromResult(userList));
            List<User>? mongo = null;
            _usersMongoRepository.Setup(x => x.InsertUserList(It.IsAny<List<User>>())).Callback<List<User>>(userList => mongo = userList);
            ...
            Assert.Equal(userList, testList);
            Assert.Equal(mongo, testList);
        }

        [Fact]
        public async Task GetUserListFromMongoDBTest()
        {
            //Arrange
            Mock<IUserRepository> _usersRepository = new Mock<IUserRepository>();
            Mock<IMongoDbRepository> _usersMongoRepository = new Mock<IMongoDbRepository>();
            ...
            _usersRepository.Setup(x => x.GetTotalUserCount()).Returns(Task.FromResult(userList.Count));
            _usersMongoRepository.Setup(x => x.GetUserCount()).Returns(Task.FromResult((long)userList.Count));
            _usersMongoRepository.Setup(x => x.GetAllUsers()).Returns(Task.FromResult(userList));
            IUserService userService = new UserService(_usersRepository.Object, _usersMongoRepository.Object);
        ...
        [Fact]
        public async Task GetUserByIdUserNullTest()
        {
            //Arrange
            Mock<IUserRepository> _usersRepository = new Mock<IUserRepository>();
            Mock<IMongoDbRepository> _usersMongoRepository = new Mock<IMongoDbRepository>();
            ...
            _usersMongoRepository.Setup(x => x.GetUserById(16).Result).Returns(buyer2);
            IUserService userService = new UserService(_usersRepository.Object, _usersMongoRepository.Object);

            //Act
            User testSubject = await userService.GetUserById(16);
            //Assert
            Assert.Equal(buyer2.Id, testSubject.Id);
        }
